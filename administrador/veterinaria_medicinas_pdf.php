...
if (@!$_SESSION['nombre']) {
  ...
  header("Location:../usuarios/veterinaria_usuarios.php");
	...
	$usuario = 'SELECT * FROM medicamentos ORDER BY id DESC';	
	$usuarios=$mysqli->query($usuario);
	
if(isset($_POST['pdf_medicamentos'])){
	...
	$content .= '
	<h1 style="text-align:center;">MEDICINAS 2019</h1>
	<br>
	<br>
		<div class="row">
        	<div class="col-md-12">
	...
	';
		...
		while ($user=$usuarios->fetch_assoc()) { 
			if($user['preciototal']==0){  $color= '#fbb2b2'; }else{ $color= '#EBFFDA'; }
	$content .= '
		<tr bgcolor="'.$color.'">
            <td>'.$user['nombre'].'</td>
            <td>'.$user['cantidad'].'</td>
            <td>'.$user['preciounitario'].'</td>
            <td>'.$user['preciototal'].'</td>
        </tr>
	';
	}
...
$r = mysqli_query($mysqli, "SELECT SUM(preciototal) AS suma FROM medicamentos ");
...
$content .= '
 ...
 <thead class="thead-dark">
   <tr>
     <th scope="col"><h2 style="text-align:center;">Total Factura $'. $f["suma"].'</h2></th>
   </tr>
 </thead>
 </table>
    	...
    	<div class="row">
              <div class="col-md-12">
              	<form method="post">
              		...
              		<div class="row">
              		<div class="col-md-12">
              		<center><button type="submit" class="img-responsive img-thumbnail" name="pdf_medicamentos"><img src="assets/images/pdf.png" width="100"></button></center>
              		<br>
              		<center><h6>Facturación De Los Medicamentos</h6></center>
              		</div>
